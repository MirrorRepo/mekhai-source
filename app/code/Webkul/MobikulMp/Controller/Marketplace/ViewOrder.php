...
// downloadable /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                                        ...
                                        $eachItem["totalPrice"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $row_total)));
                                        $eachItem["mpcodprice"] = "";
                                        if($paymentCode == "mpcashondelivery"){
                                            $eachItem["mpcodprice"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $codcharges_peritem)));
                                        }
                                        $eachItem["adminCommission"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $seller_item_commission)));
                                        $eachItem["vendorTotal"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $seller_item_cost)));
                                        $eachItem["subTotal"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $row_total)));
                                    }
                                    else{
...
// getting totals data //////////////////////////////////////////////////////////////////////////////////////////////////////////
                            ...
                            $returnArray["subTotal"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $subtotal)));
                            $returnArray["shipping"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $shippingamount)));
                            $returnArray["discount"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $totalCouponAmount)));
                            $returnArray["tax"]      = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, $totaltax)));
                            $admintotaltax           = 0;
                            $vendortotaltax          = 0;
                            if (!$taxToSeller)
                            ...
                            $returnArray["orderTotal"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, ($subtotal+$shippingamount+$codcharges_total+$totaltax-$totalCouponAmount))));
                            $returnArray["orderBaseTotal"] = "";
                            ...
                            $returnArray["vendorTotal"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, ($vendor_subtotal+$shippingamount+$codcharges_total+$vendortotaltax-$refundedShippingAmount-$couponamount))));
                            $returnArray["vendorBaseTotal"] = "";
                            if ($order->isCurrencyDifferent()) {
                            ...
                            $returnArray["adminCommission"] = $this->_helperCatalog->stripTags($order->formatPrice($this->_orderViewBlock->getOrderedPricebyorder($order, ($admin_subtotal+$admintotaltax))));
                            $returnArray["adminBaseCommission"] = "";
                            if ($order->isCurrencyDifferent()) {
...
// getting order information ////////////////////////////////////////////////////////////////////////////////////////////////////
                        if ($this->_marketplaceHelper->getSellerProfileDisplayFlag()) {
                            ...
                            if ($this->_orderViewBlock->isOrderCanShip($order)){
                                $returnArray["canShip"] = true;
                                $returnArray["shippingAddress"] = $this->_orderViewBlock->getFormattedAddress($order->getShippingAddress());
                            ...
                            if ($order->getBillingAddress()) {
                                $returnArray["billingAddress"] = $this->_orderViewBlock->getFormattedAddress($order->getBillingAddress());
                            }
                        }
                        $returnArray["paymentMethod"] = $paymentMethod;
