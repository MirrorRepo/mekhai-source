...
class MarketplaceSellerProfilePreDispatch implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        if ($this->_helper->isModuleEnable()) {
            ...
            if ($this->_request->getFullActionName() == 'marketplace_seller_profile') {
                $this->_response->setRedirect($profileUrl);
            } elseif ($this->_request->getFullActionName() == 'marketplace_seller_collection') {
                $this->_response->setRedirect($profileUrl.'collection');
            } elseif ($this->_request->getFullActionName() == 'marketplace_seller_location') {
                $this->_response->setRedirect($profileUrl.'location');
            } elseif ($this->_request->getFullActionName() == 'marketplace_seller_feedback') {
