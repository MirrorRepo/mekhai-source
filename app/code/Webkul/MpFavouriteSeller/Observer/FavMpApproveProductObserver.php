...
/**
 * Webkul MpFavouriteSeller FavMpApproveProductObserver Observer Model.
 */
class FavMpApproveProductObserver implements ObserverInterface
{
    /**
     * @var eventManager
     */
    ...
    {
        $mpProducts = $observer->getProduct();
        $seller = $observer->getSeller();
        $productId = $mpProducts->getMageproductId();
        $collection = $followersCollection = $this->_mpFavouritesellerRepository
                        ->getCustomersCollectionBySellerId($seller->getId());

        $seller = $this->_helperData->getCustomerData($seller->getId());
