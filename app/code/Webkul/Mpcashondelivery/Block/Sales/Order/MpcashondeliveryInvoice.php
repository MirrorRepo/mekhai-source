...
class MpcashondeliveryInvoice extends \Magento\Framework\View\Element\Template
{
    /**
    ...
    public function initTotals()
    {
        ...
        $title = $this->_mpcodHelper->getPymentAnountTitle();
        $store = $this->getStore();
        $invoiceCollection = $this->_order->getInvoiceCollection();
        foreach ($invoiceCollection as $previousInvoice) {
            $mpcodValue = $previousInvoice->getMpcashondelivery();
            if ((double) $mpcodValue && !$previousInvoice->isCanceled()) {
                if ($invoice->getId() == $previousInvoice->getId()) {
                    $orderPayment = $this->_order->getPayment()->getMethod();
                    if ($orderPayment == 'mpcashondelivery') {
                        $mpcod = new \Magento\Framework\DataObject(
                            [
                                'code' => 'mpcashondelivery',
                                'strong' => false,
                                'value' => $invoice->getMpcashondelivery(),
                                'base_value' => $invoice->getBaseMpcashondelivery(),
                                'label' => __($title),
                            ]
                        );
                        $parent->addTotal($mpcod, 'mpcashondelivery');
                        break;
                    }
                }
            }
